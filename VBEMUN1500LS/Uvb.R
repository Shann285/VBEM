...
##set working dir
setwd("C:/Users/dell/Desktop/MNLFAt/VB/Uniform/UN1500LS")
...
for(CIR in 1:CNUM){
  ...
  for(g in 1:NMCMC){
    ...
    for(j in 1:J){
      ...
      mulam[j] <- sig2lam[j]*( sum((y[,j]-0.5)*mueta)- 2*sum( tauksi(ksi[,j])*((munu[j]+x%*%mukappa[j,])*mueta) ) + 6/Nsig2pr )
