...
##set working dir
setwd("C:/Users/dell/Desktop/VBEMUN1500LS")
...
for(CIR in 1:CNUM){
  ...
  ##VB iteration
  for(g in 1:NMCMC){
    ppold <- 1/(1+exp(-1*( matrix(rep(munu,each=N),nrow=N)+ x%*%t(mukappa)+mueta%*%t(mulam) ) )) 

    sig2eta <- 1/( 2*rowSums(tauksi(ksi)*( matrix(rep((sig2lam+mulam^2),N),nrow=N,byr=TRUE) )) + 1/exp(x%*%mubeta-0.5*diag(x%*%sig2beta%*%t(x))) )
    mueta <- sig2eta*( rowSums((y-0.5)*matrix(rep(mulam,N),nrow=N,byr=TRUE))-2*rowSums( tauksi(ksi)*(matrix(rep(munu,N),nrow=N,byr=TRUE)+x%*%t(mukappa))*matrix(rep(mulam,N),nrow=N,byr=TRUE) )
              + (x%*%mugamma)/exp(x%*%mubeta-0.5*diag(x%*%sig2beta%*%t(x))) ) 
    ...
    for(j in 1:J){
      ...
      munu[j] <- sig2nu[j]*( sum(y[,j]-0.5)-2*sum( tauksi(ksi[,j])*(x%*%mukappa[j,]+mulam[j]*mueta) )+ Nmupr/Nsig2pr )

      sig2lam[j] <- 1/(2*sum(tauksi(ksi[,j])*(sig2eta+mueta^2)) + 1/0.5^2)
      mulam[j] <- sig2lam[j]*( sum((y[,j]-0.5)*mueta)- 2*sum( tauksi(ksi[,j])*((munu[j]+x%*%mukappa[j,])*mueta) ) + 1/0.5^2 )

      for(pp in 1:P){
        sig2kappa[j,pp] <- 1/( 2*sum(tauksi(ksi[,j])*(x[,pp]^2))+ mukdel[j,pp] )
        mukappa[j,pp] <- sig2kappa[j,pp]*( sum((y[,j]-0.5)*x[,pp])-2*sum(tauksi(ksi[,j])*x[,pp]*(munu[j]+x[,-pp]*mukappa[j,-pp]+mulam[j]*mueta)) ) 
        mukdel[j,pp] <- sqrt(mukalp[j,pp]/(sig2kappa[j,pp]+mukappa[j,pp]^2))
        ##atau2 <- 1/rinvgauss(M,mean=mukdel[j,pp],shape=mukalp[j,pp])
        mukalp[j,pp] <- (apr+1)/(bpr+0.5*(sqrt((sig2kappa[j,pp]+mukappa[j,pp]^2)/mukalp[j,pp])+1/mukalp[j,pp]))
      }
    }

    for(i in 1:N){
      for(j in 1:J){
        ksi[i,j] <- sqrt( (sig2nu[j]+munu[j]^2)+2*munu[j]*mulam[j]*mueta[i]+(sig2lam[j]+mulam[j]^2)*(sig2eta[i]+mueta[i]^2) +
    ...
    L2new <- c(munu,as.vector(t(mukappa)),mulam, mugamma,mubeta, sig2nu,as.vector(sig2kappa),sig2lam,as.vector(mukdel),as.vector(mukalp),sig2gamma,sig2beta)

    ppnew <- 1/(1+exp(-1*( matrix(rep(munu,each=N),nrow=N)+ x%*%t(mukappa)+mueta%*%t(mulam) ) ))

    if( max(abs(ppnew-ppold))<0.0001 ){break}
  }

  write(g, file="converg.txt", ncol=1, append=TRUE, sep="\t")
  write(L2new, file="L2.txt", ncol=length(L2new), append=TRUE, sep="\t")
  print(CIR)
  et<-proc.time()
  print((et-bt)[3])
